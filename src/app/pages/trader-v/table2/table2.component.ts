...
export class Table2Component implements OnInit {
  ...
  changeRowSize=event=>{
    this.selectionSize=parseInt(event);
    this.activePage=1;
    this.start=0;
    this.totalPages=parseInt(this.totalRecords)/parseInt(this.selectionSize);
    this.end=this.selectionSize-1;
  }

  gotoNext=()=>{
    this.activePage+=1;
    this.start=parseInt(this.start)+parseInt(this.selectionSize);
    this.end=parseInt(this.end)+parseInt(this.selectionSize);
...
//     }, 1000);
  ...
  filter(col: number) {
    switch (col) {
      case 1: this.dbData.sort((a, b) => {
        if (a.name < b.name) {
          return -1
        } else +1
      });
        break;
      case 2: this.dbData.sort((a, b) => b.price - a.price);
        break;
      case 3:  this.dbData.sort((a, b) => b.marketCap - a.marketCap);
        break;
      case 4:  this.dbData.sort((a, b) => b.volume - a.volume);
        break;
      case 5:  this.dbData.sort((a, b) => b.supply - a.supply);
        break;
      case 6:  this.dbData.sort((a, b) => b.percentNumber - a.percentNumber);
        break;
    }
  ...
  setData=(data)=>{
    ...
    this.dbData.map(x=>{
      ...
      let mar=data.filter(r=>r.type===x.type)[0];
