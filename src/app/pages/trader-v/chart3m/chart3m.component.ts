...
export class Chart3Componentm implements OnInit {
  ...
  setDataForChart=(data)=>
  {
    ...
    this.percentNumber=parseFloat(this.percent);
    this.arrow=data.arrow;
    this.color=data.color;
    this.data1.push(data.mktCap);
    this.data2.push(data.price);
    this.data3.push(data.time);
     let min = Math.min.apply(null, this.data2);
     let max=Math.max.apply(null,this.data1);
     let minM = Math.min.apply(null, this.data1) - Math.min.apply(null,this.data1)*.0001;;
     let maxM=Math.max.apply(null,this.data1) + Math.max.apply(null,this.data1)*.0002;
     let minP = Math.min.apply(null, this.data2) - Math.min.apply(null, this.data2)*.0001;
     let maxP=Math.max.apply(null,this.data2) + Math.max.apply(null, this.data2)*.0001;
     ...
     this.setOption(this.data1,this.data2,this.data3,minM,maxM,minP,maxP,true);
    //  this.dataOptions={
    //   series: [{
    //     data: this.data
    //   }]
  ...
  setOption(data1, data2, data3,minM,maxM,minP,maxP,animation) {
    this.chartOption = {
      animation: animation,
      grid: {
        top: 50,
