...
export class TraderVComponent implements OnInit {
  ...
  vcd_history:any={MarketDominance: 0,TotalMarketCap: 0,TotalVolume: 0};
    ...
    this._dataservice
            ...
            .subscribe(resp => {
              this.convertingValues=false;
                    ...
                    this.convertedValue=parseFloat(resp.value).toLocaleString('en-GB');
                },
                error => {
                    // console.log(error);
                    this.toastrService.show(this._configuration.ErrorFetchingResult, '',
    ...
    this._dataservice
            ...
            .subscribe(resp => {
                    ...
                    this.digitalDashMin=resp.coinHighLow.low;
                     ...
                     this.vcd_history = resp.vcd_history;
  ...
  getLiveTrade = () => {
    this.liveTradeLoading = true;
    this._dataservice.getAllOutside<any>(`https://min-api.cryptocompare.com/data/subs?fsym=BTC&tsyms=${this.tsym}`)
      .subscribe(data => {

        let btcSubs = data['USD']['TRADES'];

        this._dataservice.getAllOutside<any>(`https://min-api.cryptocompare.com/data/subs?fsym=LTC&tsyms=${this.tsym}`).subscribe(LTC => {
          let ltcSubs = LTC['USD']['TRADES'];

          this._dataservice.getAllOutside<any>(`https://min-api.cryptocompare.com/data/subs?fsym=ETH&tsyms=${this.tsym}`).subscribe(ETH => {
            let ethSubs = ETH['USD']['TRADES'];

            this._dataservice.getAllOutside<any>(`https://min-api.cryptocompare.com/data/subs?fsym=XRP&tsyms=${this.tsym}`).subscribe(XRP => {
              let xrpSubs = XRP['USD']['TRADES'];


              this.liveSubs = btcSubs.concat(ltcSubs).concat(ethSubs).concat(xrpSubs);
              this.traderService.setSubscription(this.liveSubs);
              this.traderService.getMessages().subscribe((message: any) => {
                this.loadingLiveTrade = false;
                this.liveTradeLoading = false;
                var tradeField = message.message.substr(0, message.message.indexOf("~"));
  ...
  getLiveData = () => {
    this.hubService.getMessages().subscribe((message: any) => {
      ...
      if(message.message && message.message.marquee && this.loadedFirstTime){
      ...
      if (message.message && message.message.MarketDominance)
      {
        ...
        //((Closing volume - current volume) / closing volume ) * 100
          ...
          this.MarketDominance.percentNumber=(((this.vcd_history.MarketDominance-this.MarketDominance.value)/this.vcd_history.MarketDominance)*100).toFixed(2);
          this.MarketDominance.percent=this.MarketDominance.percentNumber+"%";
      ...
      if (message.message && message.message.TotalMarketCap) {
        ...
        // console.log(this.vcd_history)
          ...
          this.TotalMarketCap.percent=(((this.vcd_history.TotalMarketCap-this.TotalMarketCap.value)/this.vcd_history.TotalMarketCap)*100).toFixed(2);
      ...
      if(message.message && message.message.TotalVolume)
      {
        this.TotalVolume = message.message.TotalVolume;
          ...
          console.log(this.vcd_history.TotalVolume);
          console.log(this.TotalVolume.value);
          this.TotalVolume.percentNumber=(((this.vcd_history.TotalVolume-this.TotalVolume.value)/this.vcd_history.TotalVolume)*100).toFixed(2);
          this.TotalVolume.percent=this.TotalVolume.percent;
  ...
  displayMyData=(data:any)=>{
    if(data.Kind==="BTC")
    {
      this.btcTrade.unshift({Type:data.Type,Market:data.Market, Price:data.Price,Quantity:data.Quantity,Total:data.Total, Coin:data.Kind});
      if(this.btcTrade.length>=50)
      this.btcTrade.splice(this.liveTrade.length-1,1)
    }
    if(data.Kind==="XRP")
    {
      this.xrpTrade.unshift({Type:data.Type,Market:data.Market, Price:data.Price,Quantity:data.Quantity,Total:data.Total, Coin:data.Kind});
      if(this.xrpTrade.length>=50)
      this.xrpTrade.splice(this.liveTrade.length-1,1)
    }
    if(data.Kind==="ETH")
    {
      this.ethTrade.unshift({Type:data.Type,Market:data.Market, Price:data.Price,Quantity:data.Quantity,Total:data.Total, Coin:data.Kind});
      if(this.ethTrade.length>=50)
      this.ethTrade.splice(this.liveTrade.length-1,1)
    }
    if(data.Kind==="LTC")
    {
      this.ltcTrade.unshift({Type:data.Type,Market:data.Market, Price:data.Price,Quantity:data.Quantity,Total:data.Total, Coin:data.Kind});
      if(this.ltcTrade.length>=50)
      this.ltcTrade.splice(this.liveTrade.length-1,1)
    }
